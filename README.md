...
| `0012_annual_goal.sql` | 目標金額を月間から年間へ（`monthly_goal_amount` → `annual_goal_amount`）。0020 で廃止 |
...
| `0020_drop_annual_goal.sql` | 年間の目標金額をやめる |
...
```
ホーム        更新ボタン / 累計貯金・前月比 / 今月のつみたて額・継続日数
              / 未精算の立替・今季の勝率 / 貯金推移（月別・年別）
貯金          試合登録・カスタム貯金 / 月間集計 / 月末確定・ワンバンク入金
              / 未登録の試合 / 記録 / 貯金ルール
