...
| `0019_sync_monthly_status_on_link.sql` | マスターが確定済みの月を、接続しているアカウントにも反映する |
...
- **つみたては強制**。試合が登録されると、マスターと接続しているアカウント全員ぶんの
  ...
  つみたてと確定は自動、入金済みだけが各自の操作です。確定の連動はマスターが押した
  時点でしか動かないため、あとから接続した人にはマスターが確定済みの月を
  接続時にまとめて反映します（`sync_monthly_status_for_user`）。作るのは確定（ready）までで、
  入金済みは実際に送金した本人しか分からないので触りません。
...
- **共有されたデータは、自分の記録と混ぜない**。相手の割り勘は割り勘タブの
  「共有されている割り勘」に、別のまとまりとして閲覧のみで出します。金額は各自のものなので
  合計には足しません（合算するのは総累計貯金額だけです）。
  貯金は、つみたてが自動で全員に立つようになったため（0018）、相手の記録を別枠で出すのを
  やめました。同じ試合が自分の記録と相手の記録で二重に並ぶだけになるためです。
- **グラフは粒度を選ばせる**。日ごとの点をそのまま並べると線が細かく揺れて、
  どの時点でいくらなのかが読めません。ホームの貯金推移は「当年の月別」と「年別」だけを出し、
  縦軸に金額、横軸に年月の目盛りを置いています。点が1つしか無いときは線を引かず、
  その値を数字で出します（データが無いわけではないため）。
